...
export default function GroundUpAppBar(props: HeadersProps) {
  ...
  const renderMobileMenu = (
    ...
    >
      <MenuItem>
        <IconButton size="large" aria-label="show notifications" color="inherit" onClick={() => history.push(USER_NOTIFICATION)}>
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
        <Toolbar>
          <IconButton size="large" edge="start" color="inherit" aria-label="open drawer" onClick={props.handleDrawerToggle} sx={{ display: { xs: "block", sm: "none" }, mr: 2 }}>
            <MenuIcon />
          </IconButton>
          <Box
            component="img"
          ...
          <Box sx={{ display: { xs: "none", md: "flex" }, alignItems: "center" }}>
            <IconButton size="large" aria-label="show Settings" color="inherit" onClick={() => history.push(USER_ADMIN_USER_MANAGEMENT)}>
              <SettingsOutlinedIcon />
            </IconButton>
            <IconButton size="large" aria-label="show notifications" color="inherit" onClick={() => toggleNotificationDrawer()}>
              <Badge badgeContent={17} color="primary">
                <NotificationsNoneIcon />
              </Badge>
            </IconButton>
